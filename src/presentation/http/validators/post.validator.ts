...
import { EPostAudience, EPostType } from '@/domain/entities/post/post.type';
...
import { PostDetailResponseDTO } from '@/presentation/http/dtos/post/post.response.dto';
import {
...
} from '@/presentation/http/exceptions/post.exception';
import { UserIsBannedException, UserNotFoundException } from '@/presentation/http/exceptions/user.exception';
...
export interface IPostValidator {
  ...
  audienceValidator: RequestHandler<GetPostDetailParamsDTO, object, object, Query, Record<string, unknown>>;
          ...
          [key]: {
            ...
            custom: {
              options: async (postId: string, { req }: Meta) => {
                ...
                const postDetail = await this.postQueryRepository.findPostDetailById(postId); // TODO: xử lý sau
  ...
  async audienceValidator(
    ...
    if (isFriendsOnlyAudience) {
      ...
      if (!isOwner && !isMention) {
        const isFriend = await this.friendsService.isFriendOf({ userId, otherUserId: ownerId });
        if (!isFriend) {
    ...
    if (!isOwner) {
      const blocked = await this.blockRepository.isBlockedEitherWay(userId, post.userId);
      if (blocked) {
