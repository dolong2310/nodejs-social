...
export abstract class BaseQueue<TData, TResult, TName extends string = string> {
  ...
  constructor({ name, queueOptions }: { name: TName; queueOptions: QueueOptions }) {
    ...
    this.queue.on('error', (err) => {
      logger.child({ module: name }).error({ err }, `${name} queue error`);
    });
  }

  abstract add(data: TData): Promise<void>;
