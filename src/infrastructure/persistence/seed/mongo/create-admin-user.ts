 ...
 *
 * Run:
 * `npm run seed:create-admin-user -- --env=development`
 *
 * Prerequisite:
 * Run `npm run seed:permissions -- --env=development` first so the ADMIN role exists.
 *
...
async function main(): Promise<void> {
  ...
  if (!adminRole) {
    throw new Error(`Role "${ERoleName.ADMIN}" not found. Run 'npm run seed:permissions -- --env=development' first.`);
  }
