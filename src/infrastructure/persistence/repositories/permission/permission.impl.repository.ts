...
import { PermissionRepositoryPort } from '@/domain/repositories/permission/permission.repository';
import {
...
import { PermissionModel } from '@/infrastructure/persistence/repositories/permission/permission.model';
  ...
  implements PermissionRepositoryPort
{
  ...
  async findPermissions({ limit, skip = 0 }: IListPermissionsInput): Promise<PermissionEntity[]> {
    const records = await this.dbCollection.find({}).skip(skip).limit(limit).toArray();
