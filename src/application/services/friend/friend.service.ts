...
export class FriendService implements IFriendService {
  ...
  async invalidateFriendCache(userId: string): Promise<void> {
    await this.redis.del(CACHE_KEYS.friends(userId));
  }
  ...
  async findFriendUserIds(userId: string): Promise<string[]> {
    const idsCached = await this.redis.get<string[]>(CACHE_KEYS.friends(userId));
    if (!idsCached) {
      const friendUserIds = await this.friendshipRepository.findFriendIdsByUserId(userId);
      await this.redis.set(CACHE_KEYS.friends(userId), friendUserIds, CACHE_TTL.FRIENDS_GRAPH);
