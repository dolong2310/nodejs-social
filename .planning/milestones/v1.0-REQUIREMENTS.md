# Requirements Archive: v1.0 brownfield v1 evolution

**Archived:** 2026-03-23
**Status:** SHIPPED

The next milestone adds a **new** `.planning/REQUIREMENTS.md` (this file is the v1.0 snapshot only). Use `/gsd-new-milestone` to define v1.1+ requirements.
...
- [x] **SOCK-01**: **Thay thế** toàn bộ luồng socket hiện tại phục vụ chat: auth từ token đã có; **identity sender chỉ từ JWT**, không tin field client.
- [x] **SOCK-02**: Join room theo **conversation** và room theo **userId** cho sự kiện cá nhân; **online-users** (hoặc tương đương) nhất quán.
- [x] **SOCK-03**: **Ghi DB trước**, emit sau cho tin nhắn và trạng thái đọc (persist-then-emit).
...
- [x] **QUAL-01**: Sửa các lỗ hổng đã biết trong `.planning/codebase/CONCERNS.md` liên quan tới **socket identity** và **persist/emit** trên đường triển khai chat.
- [x] **QUAL-02**: Thêm **test runner** và smoke tests cho luồng auth + một luồng friend hoặc chat tối thiểu (theo research STACK).
...
| SOCK-01 | Phase 6 | Complete |
| SOCK-02 | Phase 6 | Complete |
| SOCK-03 | Phase 6 | Complete |
