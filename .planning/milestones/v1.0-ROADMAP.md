...
- [x] **Phase 6: Realtime (Socket.IO)** — Replace chat socket flow; JWT identity, rooms, online presence, persist-then-emit via shared services (completed 2026-03-23)
- [x] **Phase 7: Quality hardening & tests** — Close CONCERNS gaps on implemented paths; test runner + smoke/integration coverage (completed 2026-03-23)
...
Plans:
- [x] 06-01-PLAN.md — Constants, `user:<userId>` + `chat:<id>` subscribe, remove legacy socket chat + map (SOCK-01, SOCK-02)
- [x] 06-02-PLAN.md — `IRealtimeChatEmitter`, persist-then-emit in `ChatMessagesService` for `chat:message:new` + `chat:read:updated` (SOCK-03)
- [x] 06-03-PLAN.md — Typing, friend + chat presence, D-11 doc on `notification:new` + `user:` delivery (SOCK-02)
...
Plans:
- [x] 07-01-PLAN.md — Vitest + `npm test` + HTTP integration smoke (auth verify-email + friends + chat text-only, QUAL-02, D-01, D-06)
- [x] 07-02-PLAN.md — Socket.IO JWT + `chat:subscribe` + membership signal (QUAL-02, D-03/D-04, SOCK alignment)
- [x] 07-03-PLAN.md — `07-VERIFICATION.md` + CONCERNS Resolved table (QUAL-01, D-05)
...
| 6. Realtime (Socket.IO) | 3/3 | Complete    | 2026-03-23 |
| 7. Quality hardening & tests | 3/3 | Complete    | 2026-03-23 |
